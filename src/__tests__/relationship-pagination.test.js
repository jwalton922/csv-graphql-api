...
const { CSVMetadata, FieldMetadata } = require('../schema/types');
...
describe('Relationship Pagination', () => {
  ...
  beforeAll(async () => {
    ...
    mockCSVLoader.inferSchema.mockImplementation((data, fields) => {
      return fields || [
        { name: 'id', type: 'Int' },
  ...
  describe('One-to-Many Relationship Pagination', () => {
    it('should paginate orders for a user with default pagination', async () => {
      ...
      expect((result.data )?.users.items).toHaveLength(3);
      
      // Find user 1 who has 25 orders
      const user1 = result.data?.users.items.find((user) => user.id === 1);
    ...
    it('should respect pagination limit parameter', async () => {
      ...
      const orderIds = user1.orders.map((order) => order.id).sort((a, b) => a - b);
  ...
  describe('One-to-One Relationship (No Pagination)', () => {
    it('should not add pagination parameters to one-to-one relationships', async () => {
      ...
      expect(result.errors).toBeUndefined();
      expect(result.data?.orders.items).toHaveLength(28); // 25 + 3 orders
      
      // Each order should have exactly one user
      result.data?.orders.items.forEach((order) => {
    ...
    it('should handle filtering on one-to-one relationships', async () => {
      ...
      const ordersWithActiveUsers = result.data?.orders.items.filter((order) => order.users !== null);
      const ordersWithInactiveUsers = result.data?.orders.items.filter((order) => order.users === null);
      ...
      ordersWithActiveUsers.forEach((order) => {
  ...
  describe('Pagination Edge Cases', () => {
    ...
    it('should handle user with no related records', async () => {
      ...
      const user4 = result.data?.users.items.find((user) => user.id === 4);
