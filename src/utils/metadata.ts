...
import { CSVMetadata, FieldMetadata, Relationship } from '../schema/types';

export class MetadataLoader {
  private metadataDir: string;

  constructor(metadataDir: string = 'data/metadata') {
    this.metadataDir = metadataDir;
  }

  async loadMetadata(metadataFile: string = 'metadata.yaml'): Promise<CSVMetadata[]> {
    ...
    const mainMetadata = yaml.load(mainContent) as { csvs: CSVMetadata[] };
    ...
    const mergedMetadata: CSVMetadata[] = [];
  ...
  private async loadExternalMetadata(metadataFile: string): Promise<Partial<CSVMetadata>> {
    ...
    const externalMetadata = yaml.load(content) as Partial<CSVMetadata>;

    return externalMetadata;
  }

  private mergeMetadata(base: CSVMetadata, external: Partial<CSVMetadata>): CSVMetadata {
    const merged: CSVMetadata = {
      name: base.name,
  ...
  private mergeFields(baseFields: FieldMetadata[], externalFields: FieldMetadata[]): FieldMetadata[] {
  ...
  private mergeRelationships(baseRels: Relationship[], externalRels: Relationship[]): Relationship[] {
  ...
  private normalizeMetadata(metadata: CSVMetadata): CSVMetadata {
    return {
      name: metadata.name,
      path: metadata.path,
  ...
  resolveCSVPath(csvMetadata: CSVMetadata, csvDir: string = 'data/csv'): string {
    if (csvMetadata.path) {
