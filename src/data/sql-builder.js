...
class SQLQueryBuilder {
  ...
  buildSelectQuery(tableName, filters, pagination, fields) {
  ...
  buildCountQuery(tableName, filters) {
    const safeTableName = this.database.sanitizeIdentifier(tableName);
    const params = [];
  ...
  buildFieldConditions(fieldName, filter, params) {
    const conditions = [];
  ...
  buildRelationshipQuery(fromTable, fromField, toTable, toField, fromValue, isOneToMany, filters, pagination) {
    ...
    const whereConditions = [`${safeToField} = ?`];
